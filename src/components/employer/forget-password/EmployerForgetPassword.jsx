...
import { asyncEmployerForgetPassword } from "../../../store/actions/employerActions";
...
const EmployerForgetPassword = () => {
  ...
  const sendMailHandler = async (e) => {
    try {
      e.preventDefault();
      const data = await dispatch(asyncEmployerForgetPassword(email));
console.log(data);
      navigate(`/employer/forget-link/${data.employe._id}`);
    } catch (error) {
      console.log(error);
    }
  };
...
export default EmployerForgetPassword;
